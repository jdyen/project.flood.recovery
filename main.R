...
# only load the data if a saved version isn't available
data_available <- any(grepl("data-loaded.qs", dir("data")))
if (reload_data | !data_available) {
  ...
  # hack fix for now, element 13 has null site_name column
  #   set as logical, needs to be character
  all_flow_data[[13]] <- NULL
  ...
  # save loaded data set
  qsave(flood_data_ba, file = "data/data-loaded.qs")
  
} else {
  
  flood_data_ba <- qread("data/data-loaded.qs")
...
bw_spatial <- sf::read_sf("data/blackwater_areas_2023-07-04/blackwater_areas_v2.shp")
...
pc_change_map_carp <- vic_outline %>% 
  ...
  geom_sf(
    data = pc_change_map |> 
      filter(species_group == "Common Carp") |>
      mutate(
        scaled_pc_change = abs(pc_change),
        scaled_pc_change = scaled_pc_change - min(scaled_pc_change, na.rm = TRUE),
        scaled_pc_change = scaled_pc_change / max(scaled_pc_change, na.rm = TRUE),
...
# alternative CPUE change plot based on predicted changes for each
#    waterbody, averaged over waterbodies where each species was 
#    observed in decent numbers (> 5 records)
cpue_change_filtered <- flood_impacts_spatial |>
  as_tibble() |>
  left_join(spp_to_plot_spatial, by = c("facet" = "scientific_name", "group" = "hypoxia_rank", "panel" = "waterbody")) |>
  filter(include) |>
...
cpue_plot_all_filtered <- ggplot() +
  geom_point(
    data = cpue_change_filtered,
    aes(
      x = x, 
      y = predicted, 
      group = group,
...
# repeat for YOY
cpue_change_yoy_filtered <- flood_impacts_yoy_spatial |>
  as_tibble() |>
  left_join(spp_to_plot_spatial, by = c("facet" = "scientific_name", "group" = "hypoxia_rank", "panel" = "waterbody")) |>
  filter(include) |>
  group_by(x, group, facet) |>
...
cpue_plot_yoy_filtered <- ggplot() +
  geom_point(
    data = cpue_change_yoy_filtered,
    aes(
      x = x, 
      y = predicted, 
      group = group,
...
# plot percentage change by species
pc_change_filtered <- flood_impacts_spatial |> 
  as_tibble() |>
  left_join(spp_to_plot_spatial, by = c("facet" = "scientific_name", "group" = "hypoxia_rank", "panel" = "waterbody")) |>
  filter(include) |>
  group_by(x, group, facet) |>
  ...
  mutate(
    pc_change = 100 * (after - before) / before
  ) |>
  mutate(facet = species_lookup[as.character(facet)])
pc_plot_all_filtered <- pc_change_filtered |>
  mutate(
    group = factor(
      group, 
      levels = c("L", "M", "H"),
...
# repeat for YOY
pc_change_yoy_filtered <- flood_impacts_yoy_spatial |> 
  as_tibble() |>
  left_join(spp_to_plot_spatial, by = c("facet" = "scientific_name", "group" = "hypoxia_rank", "panel" = "waterbody")) |>
  filter(include) |>
  group_by(x, group, facet) |>
  ...
  mutate(facet = species_lookup[as.character(facet)])
pc_plot_yoy_filtered <- pc_change_yoy_filtered |>
  mutate(
    group = factor(
      group, 
      levels = c("L", "M", "H"),
...
# save figs
ggsave(
  filename = "outputs/figures/cpue-all-filtered.png",
  plot = cpue_plot_all_filtered,
  device = ragg::agg_png,
  width = 8,
  height = 10,
  units = "in",
...
ggsave(
  filename = "outputs/figures/cpue-yoy-filtered.png",
  plot = cpue_plot_yoy_filtered,
  device = ragg::agg_png,
  width = 8,
  height = 5,
  units = "in",
...
ggsave(
  filename = "outputs/figures/percent-change-all-filtered.png",
  plot = pc_plot_all_filtered,
  device = ragg::agg_png,
  width = 8,
  height = 8,
  units = "in",
...
ggsave(
  filename = "outputs/figures/percent-change-yoy-filtered.png",
  plot = pc_plot_yoy_filtered,
  device = ragg::agg_png,
  width = 8,
  height = 8,
  units = "in",
